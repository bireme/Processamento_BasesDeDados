...
# -------------------------------------------------------------------------- #
# coleta_ftp.sh - Efetua coleta de dados por file transfer protocol - FTP    #
# -------------------------------------------------------------------------- #
# Chamada : coleta_ftp.sh [-V] <FI>
# Exemplo : coleta_ftp.sh bbo
# -------------------------------------------------------------------------- #
...
[ "$TIPOC" != "ftp" ] && echo "[c_ftp]  1.02      - Configuration mismatch:- Only the ftp method is supported by this program!" && exit 4
...
RSP=$?; [ "$NOERRO" = "1" ] && RSP=0
chkError $RSP "Transferindo $SIGLA por FTP"

echo "[c_rsy]  2.03      - Fixando permissoes dos objetos de $SIGLA"
for i in $(seq 0 $MAXFILE)
do
	chmod ug=rw ${FILES[$i]}
	chmod  o=r  ${FILES[$i]}
...
.   Corrente :  --
.    Chamada :  coleta_ftp.sh [-V] <ID_FI>
.    Exemplo :  nohup ../shs.lil/coleta_ftp.sh bbo &> logs/YYYYMMDD.colFTP.txt &
.Objetivo(s) :  1- Obter novos dados da Fonte de Informacao
.Comentarios :
