...
# -------------------------------------------------------------------------- #
# pos_coleta_pah.sh - Transforma o dado recebido para presaneamento          #
# -------------------------------------------------------------------------- #
# Chamada : pos_coleta_pah.sh <ID_FI>
# Exemplo : pos_coleta_pah.sh pah
...
Parameters:
 ID_FI - Identifier of Information Source to process (in this case must be pah)
...
if [ "$PARM1" != "pah" ]; then
        #     1234567890123456789012345
        echo "[pcpah]  1.01      - Erro na chamada falta o parametro 1 ou esta errado"
        echo
...
#                                            1234567890123456789012345
[ ! -s "../tabs/coletas.tab" ] && echo "[pcpah]  1.01      - Configuration error:- COLETAS table not found" && exit 3
...
#                         1234567890123456789012345
[ $N_DEB -ne 0 ] && echo "[pcpah]  0.00.01   - Testa se o indice eh valido"
IDFI=$(clANYTHING $PARM1)
[ $? -eq 0 ]     && SIGLA=$(clSIGLA $IDFI)
[ -z "$SIGLA" ]  && echo "[pcpah]  1.01      - PARM error:- PARM1 does not indicate a valid index" && exit 4

echo "[pcpah]  1.01      - Carrega definicoes da fonte para coleta de dados"
  TIPOC=$(clTYPE       $IDFI)
...
[ -z $OBJETO ] && OBJETO="LILACS.xrf;LILACS.mst" && parseFL $OBJETO && echo "[pcpah]  1.02.01   - Tentou o ajuste"

# Regra 2 se não especifica a extensao deve ser mst e xrf
egrep '\.' >/dev/null <<<$OBJETO
RSP=$?
if [ $RSP -ne 0 ]; then
	[ $TIPOC = "oai" -o $TIPOC = "dspace" ] || OBJETO=${OBJETO//;/\.\{mst,xrf\};}".{mst,xrf}" && echo "[pcpah]  1.02.02   - Extensoes ajustadas"
fi

# -------------------------------------------------------------------------- #
echo "[pcpah]  1.02.03   - Obtem os arquivos componentes (se houver)"
parseFL $OBJETO

# Determina o numero de arquivos da lista
echo "[pcpah]  1.02.04   - Quantifica componentes a obter"
i=0
while [ ! -z ${FILES[$i]} ]
do
	i=$(expr $i + 1)
...
echo "[pcpah]  1.02      - Faz corrente o diretorio de processamento"
cd $DIRETO

# Gera um Master File para PAHO com o nome pah_LILACS para prosseguir no tratamento homogeneo 
echo "[pcpah]  2         - Efetua a mudanca de ISO 2709 para M/F de ${FILES[0]}, e da outras tratativas"
echo "[pcpah]  2.01      - Preventivamente executa uma conversao DOS para UNIX"
dos2unix -f ${FILES[0]}
echo "[pcpah]  2.02      - Cria M/F para o restante do processamento"
mx iso=${FILES[0]} create=${IDFI}_LILACS -all now
RSP=$?; [ "$NOERRO" = "1" ] && RSP=0
chkError $RSP "ERROR: [pcpah] Convertendo ISO-2709 em M/F de PAHO"

echo "[pcpah]  3         - Finaliza execucao de $TREXE"
## Armazena historicamente do ISO aqui tratado
#echo "[pcpah]  3.01      - Armazena ISO coletado em diretório apropriado"
#echo "[pcpah]  3.01.01   - Garante existencia do diretorio destino para arquivos ISO-2709"
#[ -d "isos" ] || mkdir -p isos
#
#echo "[pcpah]  3.01.02   - Movimenta arquivo renomeando"
