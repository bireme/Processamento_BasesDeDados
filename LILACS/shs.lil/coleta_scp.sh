...
# -------------------------------------------------------------------------- #
# Chamada : coleta_scp.sh [-h|-i|-V|--changelog] [-d N] <FI>
...
do
	...
	echo "[c_scp]  2.01.01$i   -  Copiando ${FILES[$i]} de $SIGLA"
	egrep 'LILACS' >/dev/null <<<${FILES[$i]}
	RSP=$?
	if [ $RSP -eq 0 ]; then
		scp -P $PORTA $USERCL@$SSERVER:${SDIRETO/dbnotcertif/dbcertif}/LILACS.xrf .
	else
