...
# -------------------------------------------------------------------------- #
# pos_coleta_psi.sh - Prove transformacao no dado recebido p presaneamento   #
# -------------------------------------------------------------------------- #
# Chamada : pos_coleta_psi.sh <ID_FI>
# Exemplo : pos_coleta_psi.sh psi
# -------------------------------------------------------------------------- #
...
Parameters:
 ID_FI - Identifier of Information Source to process (in this case must be psi)
...
echo "[pcpsi]  1         - Inicia processamento de pos coleta de PSI"
# -------------------------------------------------------------------------- #
...
#                                            1234567890123456789012345
[ ! -s "../tabs/coletas.tab" ] && echo "[pcpsi]  1.01      - Configuration error:- COLETAS table not found" && exit 3
...
#                         1234567890123456789012345
[ $N_DEB -ne 0 ] && echo "[pcpsi]  0.00.01   - Testa se o indice eh valido"
IDFI=$(clANYTHING $PARM1)
[ $? -eq 0 ]     && SIGLA=$(clSIGLA $IDFI)
[ -z "$SIGLA" ]  && echo "[pcpsi]  1.01      - PARM error:- PARM1 does not indicate a valid index" && exit 4

echo "[pcpsi]  1.01      - Carrega definicoes da fonte para coleta de dados"
  TIPOC=$(clTYPE       $IDFI)
...
[ "$IDFI" != "psi" ] && echo '[pcpsi]  1.02      - Syntax error:- Only the Index PSI Periodicos I.S. can be treated by this program. $SIGLA cannot!' && exit 2

# -------------------------------------------------------------------------- #
# Ajusta lista de arquivos conforme regras gerais

# Regra 1 se não ha especificacao deve ser M/F LILACS
[ -z $OBJETO ] && OBJETO="LILACS.xrf;LILACS.mst" && parseFL $OBJETO && echo "[pcpsi]  1.01.01   - Tentou o ajuste"

# Regra 2 se não especifica a extensao deve ser mst e xrf
egrep '\.' >/dev/null <<<$OBJETO
RSP=$?
if [ $RSP -ne 0 ]; then
	[ $TIPOC = "oai" -o $TIPOC = "dspace" ] || OBJETO=${OBJETO//;/\.\{mst,xrf\};}".{mst,xrf}" && echo "[pcpsi]  1.02.02   - Extensoes ajustadas"
fi
...
echo "[pcpsi]  1.02      - Faz corrente o diretorio de processamento"
cd $DIRETO

echo "[pcpsi]  2         - Efetua a mudanca de psi.{mst,xrf} para LILACS.{mst,xrf}"
# Descompacta o M/F que vem tareado e comprimido
echo "[pcpsi]  2.01      - Testa disponibilidade do arquivo ${FILES[0]}"
[ ! -f ${FILES[0]} ] && echo "[pcpsi]  2.01.01   - Não foi possível localizar o arquivo de dados ${FILES[0]}" && exit 5 

echo "[pcpsi]  2.02      - Descomprime o M/F psi"
tar -xzvf ${FILES[0]}

# Gera uma versão denominada LILACS do M/F psi
echo "[pcpsi]  2.03      - Move e renomeia o M/F recebido"
mv home/bvspsi/bases/iah/psi.mst LILACS.mst
mv home/bvspsi/bases/iah/psi.xrf LILACS.xrf

# Limpa arvore de diretorio resultante da descompactacao
echo "[pcpsi]  3         - Finaliza execucao de $TREXE"
...
.    Entrada : PARM1 com o identificador da PSI
.      Saida : M/F LILACS gerado no diretorio psi.lil
.   Corrente : nao determinado (deve ser compensado na chamada)
.    Chamada : /bases/lilG4/shs.lil/pos_coleta_psi.sh psi
