...
# -------------------------------------------------------------------------- #
# saneamento_hom.sh - Preparo especifico da base SES para processamento iAH  #
# -------------------------------------------------------------------------- #
# Chamada : saneamento_hom.sh [opcoes] <ID_FI>
# Exemplo : nohup ../shs.lil/saneamento_hom.sh hom &> logs/$(date '+%Y%m%d').txt &
# -------------------------------------------------------------------------- #
...
Parameters:
parm1 - Information Source identifier (in this case must be: hom)
...
echo "[s_hom]  1         - Inicia processamento de coleta por scp"
# -------------------------------------------------------------------------- #
# Garante que a o parametro 1 seja informado (sai com codigo de erro 2 - Syntax Error)
if [ "$PARM1" != "hom" ]; then
        #     1234567890123456789012345
	echo "[s_hom]  1.01      - Missing or wrong IS identifier (must be: ses)"
        echo
...
#                                            1234567890123456789012345
[ $N_DEB -ne 0 ]           && echo "[s_hom]  0.00.04   - Testa se ha tabela de configuracao"
[ ! -s "../tabs/coletas.tab" ] && echo "[s_hom]  1.01      - Configuration error:- COLETAS table not found" && exit 3
...
#                         1234567890123456789012345
[ $N_DEB -ne 0 ] && echo "[s_hom]  0.00.05   - Testa se o indice eh valido"
IDFI=$(clANYTHING $PARM1)
[ $? -eq 0 ]     && SIGLA=$(clSIGLA $IDFI)
[ -z "$SIGLA" ]  && echo "[s_hom]  1.01      - PARM error:- PARM1 does not indicate a valid index" && exit 4

echo "[s_hom]  1.01      - Carrega definicoes da fonte para coleta de dados"
 DIRETO=$(clDIRETORIO  $IDFI)

# -------------------------------------------------------------------------- #
# Faz corrente o diretorio de processamento
echo "[s_hom]  1.02      - Faz corrente o diretorio de processamento"
cd $DIRETO

# Efetua a tomada dos dados

RSP=0
echo "[s_hom]  1.03      - Verifica disponibilidade do M/F ses_pre_saneamento"
