...
# -------------------------------------------------------------------------- #
# pos_coleta_med.sh - Transforma o dado recebido para presaneamento          #
# -------------------------------------------------------------------------- #
# Chamada : pos_coleta_med.sh <ID_FI>
# Exemplo : pos_coleta_med.sh med
...
Parameters:
 ID_FI - Identifier of Information Source to process (in this case must be med)
...
echo "[pcmed]  1         - Inicia processamento de pos coleta de Medcarib"
# -------------------------------------------------------------------------- #
# Garante que a o parametro 1 seja informado (sai com codigo de erro 2 - Syntax Error)
if [ "$PARM1" != "med" ]; then
        #     1234567890123456789012345
        echo "[pcmed]  1.01      - Erro na chamada falta o parametro 1 ou esta errado"
        echo
...
#                                            1234567890123456789012345
[ ! -s "../tabs/coletas.tab" ] && echo "[pcmed]  1.01      - Configuration error:- COLETAS table not found" && exit 3
...
#                         1234567890123456789012345
[ $N_DEB -ne 0 ] && echo "[pcmed]  0.00.01   - Testa se o indice eh valido"
IDFI=$(clANYTHING $PARM1)
[ $? -eq 0 ]     && SIGLA=$(clSIGLA $IDFI)
[ -z "$SIGLA" ]  && echo "[pcmed]  1.01      - PARM error:- PARM1 does not indicate a valid index" && exit 4

echo "[pcmed]  1.01      - Carrega definicoes da fonte para coleta de dados"
  TIPOC=$(clTYPE       $IDFI)
...
[ "$IDFI" != "med" ] && echo '[pcmed]  1.02      - Syntax error:- Only the Medcarib I.S. can be treated by this program. $SIGLA cannot!' && exit 2

# -------------------------------------------------------------------------- #
# Ajusta lista de arquivos conforme regras gerais

# Regra 1 se não ha especificacao deve ser M/F LILACS
[ -z $OBJETO ] && OBJETO="LILACS.xrf;LILACS.mst" && parseFL $OBJETO && echo "[pcmed]  1.02.01   - Tentou o ajuste"

# Regra 2 se não especifica a extensao deve ser mst e xrf
egrep '\.' >/dev/null <<<$OBJETO
RSP=$?
if [ $RSP -ne 0 ]; then
	[ $TIPOC = "oai" -o $TIPOC = "dspace" ] || OBJETO=${OBJETO//;/\.\{mst,xrf\};}".{mst,xrf}" && echo "[pcmed]  1.02.02   - Extensoes ajustadas"
fi

# -------------------------------------------------------------------------- #
echo "[pcmed]  1.02.03   - Obtem os arquivos componentes (se houver)"
parseFL $OBJETO

# Determina o numero de arquivos da lista
echo "[pcmed]  1.02.04   - Quantifica componentes a obter"
i=0
while [ ! -z ${FILES[$i]} ]
do
	i=$(expr $i + 1)
...
echo "[pcmed]  1.02      - Faz corrente o diretorio de processamento"
cd $DIRETO

# Gera um Master File para Medcarib com o nome med_LILACS para prosseguir no tratamento homogeneo 
echo "[pcmed]  2         - Efetua a mudanca de ISO 2709 para M/F de ${FILES[0]}, e da outras tratativas"
[ ! -s ${FILES[0]} ] && RSP=1
chkError $RSP "ERROR: [pcmed] File of addition not found"
echo "[pcmed]  2.01      - Preventivamente executa uma conversao DOS para UNIX"
dos2unix -q -f ${FILES[0]}
echo "[pcmed]  2.02      - Cria M/F para o restante do processamento"
echo "[pcmed]  2.02.01   - Carrega adicional recebido"
[ -s ${FILES[0]} ] && mx iso=${FILES[0]} create=intercooler -all now
RSP=$?; [ "$NOERRO" = "1" ] && RSP=0
chkError $RSP "ERROR: [pcmed] Loading file of addition"

echo "[pcmed]  2.02.02   - Carrega MedCarib existente"
[ ${IDFI}_LILACS.xrf ]   && rm -f ${IDFI}_LILACS.*
[ -s isos/medcarib.xrf ] && mx isos/medcarib append=${IDFI}_LILACS -all now
RSP=$?; [ "$NOERRO" = "1" ] && RSP=0
chkError $RSP "ERROR: [pcmed] Loading MedCarib pre existing"
QTDEBEFORE=$(mx ${IDFI}_LILACS count=1 "pft=f(maxmfn,1,0)/")

echo "[pcmed]  2.02.03   - Adiciona nova porcao obtida"
[ -s intercooler.xrf ]   && mx intercooler append=${IDFI}_LILACS -all now
RSP=$?; [ "$NOERRO" = "1" ] && RSP=0
chkError $RSP "ERROR: [pcmed] Consolidating medcarib"
QTDEAFTER=$(mx ${IDFI}_LILACS count=1 "pft=f(maxmfn,1,0)/")

echo "[pcmed]  2.02.04   - Armazena nova base medcarib acumulada"
mx ${IDFI}_LILACS create=isos/medcarib -all now
RSP=$?; [ "$NOERRO" = "1" ] && RSP=0
chkError $RSP "ERROR: [pcmed] Storing consolidated medcarib"

# Relata crescimento da FI
echo "[pcmed]  2.03      - Alteracao do numero de documentos na base (final - inicial): $(($QTDEAFTER - $QTDEBEFORE))"

echo "[pcmed]  3         - Finaliza execucao de $TREXE"
...
.      Saida : M/F med_LILACS gerado no diretorio med.lil
.   Corrente : nao determinado, desde que compensado na chamada
.    Chamada : /bases/lilG4/shs.lil/pos_coleta_med.sh med
