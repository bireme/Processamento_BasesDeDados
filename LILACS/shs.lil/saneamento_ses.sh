...
# -------------------------------------------------------------------------- #
# saneamento_ses.sh - Preparo especifico da base SES para processamento iAH  #
# -------------------------------------------------------------------------- #
# Chamada : saneamento_ses.sh [opcoes] <ID_FI>
# Exemplo : nohup ../shs.lil/saneamento_ses.sh ses &> logs/$(date '+%Y%m%d').txt &
# -------------------------------------------------------------------------- #
...
echo "[s_ses]  1         - Inicia processamento de coleta por scp"
# -------------------------------------------------------------------------- #
# Garante que a o parametro 1 seja informado (sai com codigo de erro 2 - Syntax Error)
if [ "$PARM1" != "ses" ]; then
        #     1234567890123456789012345
	echo "[s_ses]  1.01      - Missing or wrong IS identifier (must be: ses)"
        echo
...
#                                            1234567890123456789012345
[ $N_DEB -ne 0 ]           && echo "[s_ses]  0.00.04   - Testa se ha tabela de configuracao"
[ ! -s "../tabs/coletas.tab" ] && echo "[s_ses]  1.01      - Configuration error:- COLETAS table not found" && exit 3
...
#                         1234567890123456789012345
[ $N_DEB -ne 0 ] && echo "[s_ses]  0.00.05   - Testa se o indice eh valido"
IDFI=$(clANYTHING $PARM1)
[ $? -eq 0 ]     && SIGLA=$(clSIGLA $IDFI)
[ -z "$SIGLA" ]  && echo "[s_ses]  1.01      - PARM error:- PARM1 does not indicate a valid index" && exit 4

echo "[s_ses]  1.01      - Carrega definicoes da fonte para coleta de dados"
 DIRETO=$(clDIRETORIO  $IDFI)

# -------------------------------------------------------------------------- #
# Faz corrente o diretorio de processamento
echo "[s_ses]  1.02      - Faz corrente o diretorio de processamento"
cd $DIRETO

# Efetua a tomada dos dados

RSP=0
echo "[s_ses]  1.03      - Verifica disponibilidade do M/F ses_pre_saneamento"
...
echo "[s_ses]  1.04      - Verifica disponibilidade do gizmo de saneamento gCampo4"
[ -f "../tabs/gCampo4.xrf" ] || RSP=1
[ -f "../tabs/gCampo4.mst" ] || RSP=1
chkError $RSP "ERRO: Base gizmo gCampo4 nao pode ser localizada"

echo "[s_ses]  2         - Saneamento da base da Secretaria Estadual da Saude de Sao Paulo"
mx ${IDFI}_pre_saneamento "gizmo=../tabs/gCampo4,4" create=${IDFI}_lil_saneamento -all now tell=10000
RSP=$?; [ "$NOERRO" -eq 1 ] && RSP=0
chkError $RSP "ERRO: Saneando a base da Secretaria Estadual da Saude de Sao Paulo"
