...
# -------------------------------------------------------------------------- #
# coleta_wget.sh - Efetua coleta de dados por wget                           #
# -------------------------------------------------------------------------- #
# Chamada : coleta_wget.sh [-V] <FI>
# Exemplo : coleta_wget.sh his
#           coleta_wget.sh -d 2 xxx
# -------------------------------------------------------------------------- #
...
Opções:
 -V, --version       * Exibe a versão corrente do programa
     * Se usada interrompe a execucao do programa
...
echo "[c_gwt]  1         - Inicia processamento de coleta por rsync"
# -------------------------------------------------------------------------- #
# Garante que a FI seja informada (sai com codigo de erro 2 - Syntax Error)
if [ -z "$PARM1" ]; then
        #     1234567890123456789012345
        echo "[c_wgt]  1.01      - Erro na chamada falta o parametro 1"
        echo
...
#                                            1234567890123456789012345
[ $N_DEB -ne 0 ]               && echo "[c_wgt]  0.00.04   - Testa se ha tabela de configuracao"
[ ! -s "../tabs/coletas.tab" ] && echo "[c_wgt]  1.01      - Configuration error:- COLETAS table not found" && exit 3
unset   SIGLA
...
#                         1234567890123456789012345
[ $N_DEB -ne 0 ] && echo "[c_wgt]  0.00.05   - Testa se o indice eh valido"
IDFI=$(clANYTHING $PARM1)
[ $? -eq 0 ]     && SIGLA=$(clSIGLA $IDFI)
[ -z "$SIGLA" ]  && echo "[c_wgt]  1.01      - PARM error:- PARM1 does not indicate a valid index" && exit 4

echo "[c_wgt]  1.01      - Carrega definicoes da fonte para coleta de dados"
  TIPOC=$(clTYPE       $IDFI)
...
echo "[c_wgt]  1.02      - Efetua ajustamentos conforme regras implicitas"
# Regra 1 se nao ha especificacao deve ser M/F LILACS
[ -z $OBJETO ] && OBJETO="LILACS.xrf;LILACS.mst" && parseFL $OBJETO && echo "[c_wgt]  1.02.01   - Tentou o ajuste"
...
echo "[c_wgt]  1.03      - Faz corrente o diretorio de processamento"
cd $DIRETO
...
.   Corrente :  --
.    Chamada :  coleta_wget.sh [-V] <ID_FI>
.    Exemplo :  nohup ../shs.lil/coleta_wget.sh his &> logs/YYYYMMDD.colWGET.txt &
